...
const (
	...
	// Claude (Anthropic) OAuth 端点（参考 CLIProxyAPIPlus anthropic_auth.go）
	claudeAuthorizeURL = "https://claude.ai/oauth/authorize"
	claudeTokenURL     = "https://claude.ai/oauth/token"
	claudeClientID     = "claude-cli"

	sessionTTL = 5 * time.Minute
)

// Registrar Claude 供应商注册器实现。
...
func (r *Registrar) Register(ctx context.Context, emailAccount *email.EmailAccount, callbackSrv callback.CallbackServer, opts ...provider.RegisterOption) (*provider.RegistrationResult, error) {
	...
	// 5. TODO: 使用浏览器自动化完成 Anthropic OAuth 登录
	_ = authURL
	_ = o.Headless

	fmt.Printf("  [claude] 授权 URL: %s\n", authURL)
	fmt.Printf("  [claude] 等待 OAuth 回调...\n")
...
func (r *Registrar) Refresh(ctx context.Context, credential map[string]any) (*provider.RegistrationResult, error) {
	...
	return nil, fmt.Errorf("claude refresh: not implemented yet")
}
...
func buildAuthURL(redirectURI, state, codeChallenge string) string {
	...
	return fmt.Sprintf("%s?%s", claudeAuthorizeURL, params.Encode())
}
